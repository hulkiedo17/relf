...
static size_t read_program32_header(FILE *fp, Elf32_Phdr *header)
{
	size_t ret;

	ret = fread(header, sizeof(Elf32_Phdr), 1, fp);
	if(ret != 1)
		error(EXIT_FAILURE, errno, "fread() failed: %zu", ret);

	return ret;
}

static size_t read_program64_header(FILE *fp, Elf64_Phdr *header)
{
	size_t ret;

	ret = fread(header, sizeof(Elf64_Phdr), 1, fp);
	if(ret != 1)
		error(EXIT_FAILURE, errno, "fread() failed: %zu", ret);
...
static void print_program64_header(Elf64_Phdr *header)
{
	...
	/*
	printf("\tFilesz = %#0lx", hdr->p_filesz);
	printf("\tMemsz = %#0lx", hdr->p_memsz);*/
}
...
Elf32_Phdr* read_program32_headers(const char *filename, Elf32_Ehdr *elf_header)
{
	...
	if(!fp)
		error(EXIT_FAILURE, errno, "cannot open \'%s\' file", filename);

	program_headers_offset = (ssize_t)elf_header->e_phoff;
	fseek(fp, program_headers_offset, SEEK_SET);

	program_headers = malloc(sizeof(Elf32_Phdr) * elf_header->e_phnum);
	if(!program_headers)
		error(EXIT_FAILURE, errno, "cannot allocate memory");

	for(size_t i = 0; i < elf_header->e_phnum; i++)
	{
		read_program32_header(fp, &program_headers[i]);
	}
...
Elf64_Phdr* read_program64_headers(const char *filename, Elf64_Ehdr *elf_header)
{
	...
	for(size_t i = 0; i < elf_header->e_phnum; i++)
	{
		read_program64_header(fp, &program_headers[i]);
	}
